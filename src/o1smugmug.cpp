...
{
    const char *const strings[] = {"Public", "Full"};
    const std::size_t stringsSize = sizeof(strings) / sizeof(strings[0]);
    Q_ASSERT(
        access >= 0 && static_cast<std::size_t>(access) < stringsSize && "Unsupported SmugMug authorization access!");
...
{
    const char *const strings[] = {"Read", "Add", "Modify"};
    const std::size_t stringsSize = sizeof(strings) / sizeof(strings[0]);
...
{
    setAuthorizeUrl(QUrl(::basicAuthorizationUrl() + "?Access=" + ::accessToString(access) +
