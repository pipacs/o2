...
O2ReplyServer::O2ReplyServer(QObject *parent): QTcpServer(parent),
  timeout_(15), maxtries_(3), tries_(0) {
    o0debug() << "O2ReplyServer: Starting";
...
void O2ReplyServer::onIncomingConnection() {
    o0debug() << "O2ReplyServer::onIncomingConnection: Receiving...";
...
void O2ReplyServer::onBytesReady() {
    ...
    o0debug() << "O2ReplyServer::onBytesReady: Processing request";
    // NOTE: on first call, the timeout timer is started
    QTcpSocket *socket = qobject_cast<QTcpSocket *>(sender());
    if (!socket) {
        o0warning() << "O2ReplyServer::onBytesReady: No socket available";
        return;
    }
    QByteArray reply;
    ...
    socket->write(reply);
    o0debug() << "O2ReplyServer::onBytesReady: Sent reply";

    QByteArray data = socket->readAll();
    QMap<QString, QString> queryParams = parseQueryParams(&data);
    if (queryParams.isEmpty()) {
        if (tries_ < maxtries_ ) {
            o0debug() << "O2ReplyServer::onBytesReady: No query params found, waiting for more callbacks";
            ++tries_;
            return;
        } else {
            tries_ = 0;
            o0warning() << "O2ReplyServer::onBytesReady: No query params found, maximum callbacks received";
            closeServer(socket, false);
            return;
        }
    }
    o0debug() << "O2ReplyServer::onBytesReady: Query params found, closing server";
...
QMap<QString, QString> O2ReplyServer::parseQueryParams(QByteArray *data) {
    o0debug() << "O2ReplyServer::parseQueryParams";

    //o0debug() << QString("O2ReplyServer::parseQueryParams data:\n%1").arg(QString(*data));
...
void O2ReplyServer::closeServer(QTcpSocket *socket, bool hasparameters)
{
  ...
  o0debug() << "O2ReplyServer::closeServer: Initiating";
  int port = serverPort();

  if (!socket && sender()) {
      QTimer *timer = qobject_cast<QTimer*>(sender());
      if (timer) {
          o0warning() << "O2ReplyServer::closeServer: Closing due to timeout";
          timer->stop();
  ...
  if (socket) {
      ...
      if (timer) {
          o0debug() << "O2ReplyServer::closeServer: Stopping socket's timeout timer";
          timer->stop();
      }
      socket->disconnectFromHost();
  }
  close();
  o0debug() << "O2ReplyServer::closeServer: Closed, no longer listening on port" << port;
